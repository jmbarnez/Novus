...
function TargetPanel.draw(world, player)
    ...
    elseif target.station then
        nameText = "Station"
    elseif target.item and target.item.name then
        nameText = target.item.name
    else
        nameText = "Target"
    end
    ...
    -- Compute distance from local ship to target (if possible)
    ...
    if isStation then
        if hasDistance then
            ...
            love.graphics.setColor(Theme.colors.textPrimary)
            local distanceText = string.format("Distance: %.0f", distanceValue)
            if inRange then
        ...
        barY = infoY + 4
    elseif hasDistance then
        -- Non-station targets: just display distance
        love.graphics.setColor(Theme.colors.textPrimary)
        local distanceText = string.format("Distance: %.0f", distanceValue)
        love.graphics.print(distanceText, cx, infoY)
        barY = infoY + textHeight + 4
    end
