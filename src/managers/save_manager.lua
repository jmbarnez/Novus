...
function SaveManager.build_snapshot(world, player)
    ...
    if Config and Config.BACKGROUND then
        graphics.nebula = Config.BACKGROUND.ENABLE_NEBULA ~= false
    end
...
function SaveManager.apply_snapshot(world, player, ship, snapshot)
    ...
    if snapshot.audio then
        local a = snapshot.audio
        if a.master ~= nil and love and love.audio and love.audio.setVolume then
            local v = a.master
            if v < 0 then v = 0 end
            if v > 1 then v = 1 end
            love.audio.setVolume(v)
        end
        if a.music ~= nil and SoundManager and SoundManager.set_music_volume then
            SoundManager.set_music_volume(a.music)
        end
        if a.sfx ~= nil and SoundManager and SoundManager.set_sfx_volume then
            SoundManager.set_sfx_volume(a.sfx)
        end
    end

    if snapshot.graphics and Config and Config.BACKGROUND then
        local g = snapshot.graphics
        if g.nebula ~= nil then
            Config.BACKGROUND.ENABLE_NEBULA = g.nebula and true or false
        end
    end
