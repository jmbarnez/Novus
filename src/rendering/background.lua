...
function Background:draw(cam_x, cam_y, cam_sector_x, cam_sector_y)
    ...
    if Config.BACKGROUND.ENABLE_NEBULA ~= false then
        ...
        if self.nebulaShader and self.nebulaParams then
            ...
            pcall(self.nebulaShader.send, self.nebulaShader, "offset", { offset_x, offset_y })
            self.nebulaShader:send("resolution", { sw, sh })
            pcall(self.nebulaShader.send, self.nebulaShader, "noiseScale", self.nebulaParams.noiseScale or 1.0)
            pcall(self.nebulaShader.send, self.nebulaShader, "flow", self.nebulaParams.flow)
