...
function EntityUtils.apply_damage(entity, damage, attacker, world)
    ...
    if entity.transform and world then
        ...
        if world.local_ship then
            show_text = (entity == world.local_ship or attacker == world.local_ship)
        end
        if show_text then
...
function EntityUtils.apply_ship_damage(entity, damage, attacker, world)
    ...
    if entity.shield then
        ...
        if current > 0 then
            ...
            if shield_damage > 0 and love and love.timer and love.timer.getTime then
                shield.last_hit_time = love.timer.getTime()
            end
        end
    end
