...
local function interpolateRemoteEntity(self, entity, state, server_time)
    ...
    -- Use local receipt time for interpolation. We don't need absolute
    -- server timestamps here; we only require monotonic times so
    -- interpolation/extrapolation can operate correctly.
    local packet_time = love.timer.getTime()
...
local function syncNetworkEntity(self, state, server_time)
    ...
    if entity then
        ...
        -- First time we see this entity from the network, spawn it locally.
...
function PlayNetwork.initNetwork(self, is_joining, join_host)
    ...
    Client.setExplosionCallback(function(packet) PlayNetwork.handleExplosion(self, packet) end)
...
function PlayNetwork.handleExplosion(self, packet)
    if not self.world or self.world.hosting then
        return
    end

    local fx_type = packet.fx_type or "ship"
    if fx_type ~= "ship" then
        return
    end

    local x = packet.x or 0
    local y = packet.y or 0
    local sx = packet.sx or 0
    local sy = packet.sy or 0
    local radius = packet.radius or 16
    local color = packet.color or { 1.0, 0.75, 0.35, 1.0 }
...
function PlayNetwork.updateInterpolation(self, dt)
    ...
    for id, buffer in pairs(self.world.interpolation_buffers) do
        ...
        if entity then
            ...
            -- Remote ships: keep a bit of extra delay for smoother motion
            if entity.vehicle and id ~= self.my_entity_id then
                effective_delay = delay + 0.03
            -- Small, fast-moving entities (items & chunks): reduce delay so they
            -- don't appear to trail far behind their actual position.
            elseif entity.item or entity.asteroid_chunk then
                -- Use a much smaller delay for responsiveness; cap at 0.10s so
                -- they stay close to real time even on higher pings.
                effective_delay = math.min(delay, 0.10)
            end
...
function PlayNetwork.startHosting(self)
    ...
    if Server.start(25565, self.world) then
        ...
        self.world:on("entity_died", function(entity)
            if entity and entity.vehicle and entity.transform and entity.sector then
                local radius = (entity.render and entity.render.radius) or 16
                local color = (entity.render and entity.render.color) or { 1.0, 0.75, 0.35, 1.0 }
                local packet = Protocol.createExplosionFxPacket(
                    "ship",
                    entity.transform.x or 0,
                    entity.transform.y or 0,
                    entity.sector.x or 0,
                    entity.sector.y or 0,
                    radius,
                    color
                )
                Server.broadcast(packet)
