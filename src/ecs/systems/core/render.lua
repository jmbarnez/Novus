        ...
        for _, e in ipairs(self.drawPool) do
            ...
            if math.abs(diff_x) <= 1 and math.abs(diff_y) <= 1 then
                ...
                if camera then
                    ...
                    local basePadding = 64
                    local radius = (r and r.radius) or 0
                    local padding = basePadding + radius
                ...
                if e.station_area then
                    love.graphics.push()
                    local zoom = camera and (camera.scale or 1) or 1
                    local prevLineWidth = love.graphics.getLineWidth()
                    local baseWidth = 2
                    local lineWidth = baseWidth
                    if zoom > 0 then
