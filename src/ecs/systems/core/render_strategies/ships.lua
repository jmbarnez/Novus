...
local shieldHitShader = nil

local function ensureShieldHitShader()
    if shieldHitShader then
        return shieldHitShader
    end

    local shader_path = "assets/shaders/shield_hit.glsl"
    if love.filesystem.getInfo(shader_path) then
        shieldHitShader = love.graphics.newShader(shader_path)
    end

    return shieldHitShader
end

local function applyShieldHitShader(e)
    if not e or not e.shield then
        return nil
    end

    local shield = e.shield
    if not shield.last_hit_time then
        return nil
    end
    ...
    local elapsed = now - shield.last_hit_time
    local duration = 0.25
    ...
    local shader = ensureShieldHitShader()
    if not shader then
        return nil
    end

    local t = elapsed / duration
    local intensity = 1.0 - t
    intensity = intensity * intensity

    shader:send("hit_intensity", intensity)
    shader:send("hit_time", elapsed)
    shader:send("shield_color", { 0.20, 0.95, 1.00 })
...
local function resetShader(shader)
    if shader then
...
function ShipsRender.ship(e)
    ...
    local activeShader = applyShieldHitShader(e)
