...
local function collect_item(item, collector, world)
	...
	local is_local_collector = world and world.local_ship and (collector == world.local_ship)
        ...
        if collector.cargo then
            ...
            if (cargo.current + item_vol) <= cargo.capacity then
				...
				if world and collector.transform then
					...
					if show_text then
						local vol_text = string.format("+%.1f m3 %s", item_vol, tostring(itemComp.name))
    ...
    if is_local_collector then
		SoundManager.play_sound("item_pickup")
	end
