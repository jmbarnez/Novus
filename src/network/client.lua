...
function Client.onReceive(peer, data)
    ...
    elseif packet.type == Protocol.PacketType.EXPLOSION_FX then
        if Client.onExplosion then
            Client.onExplosion(packet)
        end
    end
end
...
function Client.setExplosionCallback(fn)
    Client.onExplosion = fn
end
