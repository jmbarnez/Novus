...
local function hasEntityStateChanged(prev, cur)
    ...
    -- Tighter thresholds for small, fast-moving or magnet-influenced entities
    local posEps = 0.5
    local velEps = 0.5
    if cur and (cur.type == "item" or cur.type == "asteroid_chunk") then
        posEps = 0.15
        velEps = 0.15
    end

    if diff(prev.x, cur.x, posEps) or diff(prev.y, cur.y, posEps) or diff(prev.r, cur.r, 0.01) then
        return true
    end

    if diff(prev.vx, cur.vx, velEps) or diff(prev.vy, cur.vy, velEps) then
        return true
    end
