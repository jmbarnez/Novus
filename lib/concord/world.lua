...
function World:emit(functionName, ...)
    ...
    local profiler = self.__profiler
    local profEnabled = profiler and profiler.enabled
    local profBucket = profEnabled and (profiler.bucket or "default") or nil
        ...
        for i = 1, #listeners do
            ...
            if (listener.system.__enabled) then
                ...
                if profEnabled then
                    ...
                    local dtMs = (love.timer.getTime() - t0) * 1000

                    local buckets = profiler.buckets
                    if not buckets then
                        buckets = {}
                        profiler.buckets = buckets
                    end

                    local bucket = buckets[profBucket]
                    if not bucket then
                        bucket = {}
                        buckets[profBucket] = bucket
                    end

                    local ev = bucket[functionName]
                    if not ev then
                        ev = {}
                        bucket[functionName] = ev
                    end

                    local sysName = listener.system.__profileName or tostring(listener.system)
                    ev[sysName] = (ev[sysName] or 0) + dtMs
                else
                    listener.callback(listener.system, ...)
                end
            end
